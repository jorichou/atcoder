...
if __name__ == "__main__": # メイン処理
    ...
    for _ in range(n - 1):
        cost = acin.list()
        for weight in range(g, n + 1):
            costs[(s, weight)] = cost[weight - g]
        s += 1
        g += 1
    if flag == 0:
        for start in range(1, n):
            for goal in range(3, n + 1):
                if goal - start >= 2:
                    for cent in range(start+1, goal):
                        if costs[(start, goal)] > costs[(start, cent)] + costs[(cent, goal)]:
                            print('Yes')
                            sys.exit(0)
    print('No')