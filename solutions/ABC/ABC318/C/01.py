...
def main():
    ...
    f = i_list()
    f.sort()
    f_cu = list(itertools.accumulate(f))
    count = 0
    i = -1
    k = math.ceil(n / d)
    # print(k)
    results = [p * k]
    for k in range(0, math.ceil(n / d)):
        results.append(k * p + f_cu[n - 1 - k * d])
    print(min(results))
    # else:
    #     while i - d >= -n:
    #         if f_cu[i] - f_cu[i - d] >= p:
    #             count += 1
    #             i -= d
    #         else:
    #             break
    ...
    #     k = x.pop(0)
    #     for comb in c:
    #         if comb[0] in x and comb[1] in x:
    #             c.remove(comb)
    # print(c)
    # o_judge(c == [])
    return
