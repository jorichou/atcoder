...
if __name__ == "__main__": # メイン処理
    ...
    n, q = aci.multiple_input(int)
    x = aci.list_input(int)
    boxes = [0 for _ in range(n)] # 箱に入っているボール
    index = []
    ...
    for query in x:
        if query >= 1:
            ...
            minimum = min(boxes)
            target_box = boxes.index(minimum)
            boxes[target_box] += 1
            index.append(target_box + 1)

        else:
            pass

    aco.list_output(index)